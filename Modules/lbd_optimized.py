...
class LineMatcherOptimized:
    ...
    def match_lines(self,
        ...
        return matches
    def match_by_descriptor(self,
                            descriptors1: np.ndarray,
                            descriptors2: np.ndarray) -> List[Tuple[int, int]]:
        """
        Match lines using descriptors only (no geometric checks).
        
        Use this when matching 3D map lines against 2D frame lines,
        where geometric consistency checks (midpoint proximity, angle
        similarity) are meaningless because the line sets live in
        different coordinate spaces.
        
        Outlier rejection is left to RANSAC in the pose solver.
        
        Args:
            descriptors1: (N, D) descriptors from set 1 (e.g. map lines)
            descriptors2: (M, D) descriptors from set 2 (e.g. current frame)
            
        Returns:
            List of (idx1, idx2) matches, with Lowe's ratio test applied.
        """
        if len(descriptors1) == 0 or len(descriptors2) == 0:
            return []
        ...
        for i in range(len(desc1_norm)):
            # Descriptor distances to all candidates
            desc_distances = np.linalg.norm(desc1_norm[i] - desc2_norm, axis=1)

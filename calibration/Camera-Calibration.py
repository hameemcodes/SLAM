...
def calculate_reprojection_error(objpoints, imgpoints, mtx, dist, rvecs, tvecs):
    ...
    Returns:
        ...
        errors: List of per-image reprojection errors
    """
    total_error = 0
    errors = []
...
def plot_reprojection_errors(errors):
    """Plot reprojection errors as a bar chart."""
    plt.figure(figsize=(10, 6))
    plt.bar(range(1, len(errors) + 1), errors, color='steelblue')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIRECTORY, 'reprojection_errors.png'), dpi=150)
    plt.close()
    print(f"Reprojection error plot saved to {os.path.join(OUTPUT_DIRECTORY, 'reprojection_errors.png')}")
...
def main():
    ...
    mean_error, errors = calculate_reprojection_error(objpoints, imgpoints, mtx_downscaled, dist, rvecs, tvecs)
    plot_reprojection_errors(errors)
